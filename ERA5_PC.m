...
for yearsIndex = 1:length(years)
    startIndex=1;
    outFile=[qDir 'Q1_' num2str(years(yearsIndex)) '.nc'];
...
[~,~,~,~,O1,O2,eof]=ERA5_EOF_decomposition(Omega,level);

%save the years's PCs to the matrix of all the PCs
O1tot=cat(1,O1tot,O1);
O2tot=cat(1,O2tot,O2);

%save this years PCs
save([baseDir 'PC/' 'ERA5_PC' fileAppend(1:5) '.mat'],'O1','O2');
Q1=cat(1,Q1,Omega.*permute(repmat(dS_bar_tot_dp,[1,1,1,size(Omega,1)]),[4 1 2 3]));
